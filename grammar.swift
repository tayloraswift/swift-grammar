...
@resultBuilder
struct BufferInterpolation<Codec>:ExpressibleByStringInterpolation, StringInterpolationProtocol 
    where Codec:UnicodeCodec 
{
    private(set)
    ...
    init(stringLiteral:String)
    {
        ...
        self.appendLiteral(stringLiteral)
    }
    ...
    {
        for scalar:Unicode.Scalar in literal.unicodeScalars 
        {
            Codec.encode(scalar)
            {
    ...
    mutating 
    func appendInterpolation<T>(_ value:T, radix:Int) 
        where T:BinaryInteger 
    {
        self.appendLiteral(String.init(value, radix: radix))
    }
    mutating 
    func appendInterpolation<T>(padding value:T, left:Int, radix:Int) 
        where T:BinaryInteger 
    {
        self.appendLiteral(String.init(padding: value, left: left, radix: radix))
    }
    mutating 
    func appendInterpolation<S>(_ elements:S) 
        where S:Sequence, S.Element == Codec.CodeUnit
    {
        self.buffer.append(contentsOf: elements)
    }
    mutating 
    func appendInterpolation<T>(_ value:T) 
        where T:Grammar.Serializable, T.Terminals.Element == Codec.CodeUnit
    {
        self.buffer.append(contentsOf: value.serialized)
    }
    mutating 
    func appendInterpolation<T>(_:T.Type) 
        where T:Grammar.TerminalSequence, T.Terminals.Element == Codec.CodeUnit
    {
        self.buffer.append(contentsOf: T.terminals)
    }
}
extension BufferInterpolation where Codec.CodeUnit == UInt8 
{
    mutating 
    func appendInterpolation(latin1:String) 
    {
        for scalar:Unicode.Scalar in latin1.unicodeScalars 
        {
            if let value:UInt8 = .init(exactly: scalar.value)
            {
    ...
    mutating 
    func appendInterpolation(latin1:Character) 
    {
        for scalar:Unicode.Scalar in latin1.unicodeScalars 
        {
            if let value:UInt8 = .init(exactly: scalar.value)
...
extension BufferInterpolation
{
    static 
    func buildExpression(_ value:Self) -> [Codec.CodeUnit]
    {
        value.buffer 
    }
    static 
    func buildBlock(_ interpolations:[Codec.CodeUnit]...) -> [Codec.CodeUnit]
    {
        .init(interpolations.joined())
    }
    static 
    func buildArray(_ interpolations:[[Codec.CodeUnit]]) -> [Codec.CodeUnit]
    {
        .init(interpolations.joined())
    } 
} 
